...
public class ShopProfileController(SpazaSureDbContext db, IFileStorageService storage) : ControllerBase
{
    ...
    public async Task<IActionResult> UploadDocument([FromForm] string docType, IFormFile file)
    {
        ...
        var key = $"shop-docs/{shop.Id}/{fileName}";

        string docUrl;
        await using (var stream = file.OpenReadStream())
        {
            docUrl = await storage.SaveAsync(stream, key, file.ContentType, HttpContext.RequestAborted);
        }
